...
public class MainActivity extends ActionBarActivity {
	...
	Callback callback = new Callback() {
		...
		public boolean handleMessage(Message msg) {			
			...
			if(tienThuong > 0){
				Toast.makeText(getApplicationContext(), "Quá dữ thím trúng được " + tienThuong, Toast.LENGTH_SHORT).show();
...
public void LacXiNgau(View v){
	...
	if (kiemtra == 0) {
		Toast.makeText(getApplicationContext(), "Đặt cược đi thím hai ! ", Toast.LENGTH_SHORT).show();
	}else {
		if(kiemtra > tongtiencu){
			Toast.makeText(getApplicationContext(), "Không đủ tiền cược thím hai ! ", Toast.LENGTH_SHORT).show();			
		}else {
